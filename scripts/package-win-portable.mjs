...
const productName = packageJson.build?.productName ?? 'OBS 音频守卫';
...
const unpackDir = join(releaseDir, `OBS-Audio-Guard-${appVersion}-win-x64-portable`);
const appDir = join(unpackDir, 'resources', 'app');
const zipPath = join(releaseDir, `OBS-Audio-Guard-${appVersion}-win-x64-portable.zip`);
