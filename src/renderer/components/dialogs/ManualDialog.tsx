      ...
      <section className="manual-dialog">
        ...
        <div className="manual-sections">
          ...
          <article>
            ...
            <p>打开设置 → OBS 连接,确认主机为 127.0.0.1。连接成功后,在设置 → 检测与报警里选择主播麦克风、无线领夹麦、声卡输入或直播主混音。图片、文字、显示器采集等无声音源会自动过滤。</p>
          </article>
          <article>
            <strong>3. 设置静音报警规则</strong>
            <p>在设置 → 检测与报警中调整静音时长(秒)、预警比例和静音阈值(dB)。默认连续静音 120 秒报警,90 秒时先预警。电商直播可根据口播密度调整时间,阈值一般保持 -55 dB。</p>
          </article>
          <article>
            <strong>4. 不开播时测试</strong>
            <p>OBS 已连接但还没推流时,可以在设置 → 诊断测试中打开"模拟开播检测"。这样可以测试静音计时、电平表和报警弹窗。测完后关闭模拟开播。</p>
          </article>
          <article>
