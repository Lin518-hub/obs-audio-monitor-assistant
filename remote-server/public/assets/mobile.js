...
let device = null, requestId = null, accessToken = null, socket = null, state = null, pollTimer = null;
...
const levelPct = (db) => db == null ? 0 : Math.max(0, Math.min(100, ((db + 90) / 90) * 100));

async function start() {
  if (!pairToken) return show('pair-view');
  try {
...
async function openDashboard() {
  ...
  device = session.device; state = session.state; show('dashboard-view'); render(); connectSocket();
}
...
function connectSocket() {
  ...
  socket.onmessage = (event) => {
    ...
    if (message.type === 'command-result') toast(message.ok ? '操作已执行' : message.message || '操作失败');
  };
  socket.onclose = (event) => { setOnline(false); if (event.code === 4003) { localStorage.removeItem(`obsRemoteAccess:${device.uuid}`); location.reload(); return; } setTimeout(connectSocket, 2500); };
}
...
function render() {
  ...
  $('audio-level').style.width = `${levelPct(audio.levelDb)}%`; $('audio-threshold').style.left = `${levelPct(audio.thresholdDb)}%`; $('audio-hint').textContent = audio.hint || '等待状态'; $('audio-silence').textContent = audio.silentForSeconds ? `${audio.silentForSeconds}s` : '';
  ...
  $('auto-transition').disabled = !atem.connected || !atem.previewInput; renderInputs(atem);
}

function renderInputs(atem) {
  const root = $('atem-inputs'); root.replaceChildren();
  for (const input of atem.inputIds || []) {
    const button = document.createElement('button'); button.className = `input-button ${input===atem.programInput?'program':''} ${input===atem.previewInput?'preview':''}`;
    const strong = document.createElement('strong'); strong.textContent = atem.inputLabels?.[input] || `Input ${input}`;
    const span = document.createElement('span'); span.textContent = input===atem.programInput?'正在播出':input===atem.previewInput?'当前预览':'点击选入 PVW';
    button.append(strong,span); button.addEventListener('click',()=>sendCommand('atem.preview',{input})); root.append(button);
  }
